...
class WXBot:
	...
	def proc_at_info(msg):
		...
		if len(segs) > 1:
			...
			if segs:
				pm = re.search(u'@.* ', segs[0])
				pm = pm.group() if pm is not None else None
				if pm:
					name = pm[1:-1]
					str_msg_all = str_msg = segs[0].replace(pm, '')
					infos.append({'type': 'at', 'value': name})
				else:
					infos.append({'type': 'str', 'value': segs[-1]})
					str_msg_all = msg
	...
	def covert_voice_to_text(self, file_name, msg_content, msg, user, msg_type_id):#语音识别
	...
	def voice_to_text_complete(self, res):
		import ast
		...
		if not result:
			return
		print 'result:::', result
		self.temp_msg_content['desc'] = result
		self.temp_msg_content['data'] = result
		self.temp_msg_content['type'] = 0
		message = {'msg_type_id': self.temp_msg_type_id,
					'msg_id': self.temp_msg['MsgId'],
					'content': self.temp_msg_content,
					'to_user_id': self.temp_msg['ToUserName'],
					'user': self.temp_user,
					'voice_to_text': True}
		self.handle_msg_all(message)
		# pass

	def extract_msg_content(self, msg_type_id, msg, user = None):
		"""
		content_type_id:
			0 -> Text
			1 -> Location
		...
		if mtype == 1:
			...
			if self.DEBUG:
				...
				self.covert_voice_to_text(voice, msg_content, msg, user, msg_type_id)
				return None
	...
	def handle_msg(self, r):
		...
		for msg in r['AddMsgList']:
			...
			content = self.extract_msg_content(msg_type_id, msg, user = user)
			if content is None:
				return
